...
class DartProperty extends Equatable {
  DartProperty({
    ...
    type = converDartType(keyValuePair.value.runtimeType);
  ...
  String getTypeString({String? className}) {
    ...
    if (result != null) {
      result = stringFormat(result, <String>[
        className ??
            getDartTypeString(converDartType(temp?.runtimeType ?? Object), this)
      ]);
    }

    return result ?? (className ?? getDartTypeString(type, this));
  }
  ...
  String getBaseTypeString({String? className}) {
    ...
    return getDartTypeString(converDartType(temp?.runtimeType ?? Object), this);
  }
