...
class JsonToDartController extends ChangeNotifier {
  ...
  final TextEditingController _textShowController = TextEditingController();

  TextEditingController get textShowController => _textShowController;
  ...
  void formatJson() {
    ...
    try {
      ...
      dynamic jsonData = jsonDecode(inputText);
  ...
  void generateDart() {
    ...
    if (dartObject != null) {
      ...
      try {
        ...
        if (ConfigSetting().addMethod) {
          if (ConfigSetting().enableArrayProtection) {
            ...
            sb.writeLine(ConfigSetting().nullsafety
                ? tryCatchMethodNullSafety
                : tryCatchMethod);
          }

          sb.writeLine(ConfigSetting().enableDataProtection
              ? ConfigSetting().nullsafety
                  ? asTMethodWithDataProtectionNullSafety
                  : asTMethodWithDataProtection
              : ConfigSetting().nullsafety
                  ? asTMethodNullSafety
                  : asTMethod);
        }
        ...
        _textShowController.text = result;
        Clipboard.setData(ClipboardData(text: result));
        showToast(I18n.instance.generateSucceed);
      } catch (e, stack) {
        print('$e');
        print('$stack');
        _textShowController.text = sb.toString();
